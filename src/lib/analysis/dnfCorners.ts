...
function checkIsSolved(pattern: KPattern) {
  ...
  for (let i = 0; i < 8; i++) {
    const positionMatches = corners.pieces[i] == i;
    const orientationMatches = corners.orientation[i] == 0;
...
export async function analyseCorners(solve: Solve): Promise<SolveAnalysis> {
