    ...
    if (count >= 5) {
      if (dnfCount <= 1) {
        ...
        if (validTimes.length >= count - 1) {
          validTimes.sort((a, b) => a - b);
          const trimmedTimes = validTimes.slice(1, -1);
          avgTime = trimmedTimes.reduce((acc, time) => acc + time, 0) / trimmedTimes.length;
        }
      }
    } else {
      if (dnfCount === 0) {
        const validTimes = times.filter(time => time !== null) as number[];
        avgTime = validTimes.reduce((acc, time) => acc + time, 0) / validTimes.length;
      }
    }
...
function StatItem({ label, current, best }: { label: string; current: string; best: string }) {
  return (
    <div className="flex justify-between items-center py-1">
      <span className="text-sm text-muted-foreground font-medium">{label}</span>
      <div className="flex gap-4">
        <span className="font-mono text-sm font-medium w-16 text-right">{current}</span>
        <span className="font-mono text-sm font-medium w-16 text-right">{best}</span>
      </div>
    </div>
  );
}
...
export default function ResultsStats() {
  ...
  const relevantAverages = averageConfigs.filter(config => solveCount >= config.count);
  ...
  return (
    <div className="flex flex-col gap-2 p-4">
      ...
      <div className="text-center">
        <div className="font-semibold text-sm">Solves: {successes}/{solveCount} ({((successes / solveCount) * 100).toFixed(2)}%)</div>
