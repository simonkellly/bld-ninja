...
async function analyse() {
  ...
  const algStats = new Map<
    string,
    { count: number; times: number[]; type: string }
  >();
  ...
  const algorithmsWithSufficientData = algAnalysis.filter(
    alg => alg.count >= 2
  );

  // Separate by algorithm type
  const edgeAlgorithms = algorithmsWithSufficientData.filter(
    alg => alg.type === 'Edge'
  );
  const cornerAlgorithms = algorithmsWithSufficientData.filter(
    alg => alg.type === 'Corner'
  );
  ...
  const edgeAlgorithmsForFastest = allAlgorithmsForFastest.filter(
    alg => alg.type === 'Edge'
  );
  const cornerAlgorithmsForFastest = allAlgorithmsForFastest.filter(
    alg => alg.type === 'Corner'
  );
  ...
  console.log(
    `   • ${edgeAlgorithms.length} edge algorithms with 2+ occurrences`
  );
  console.log(
    `   • ${cornerAlgorithms.length} corner algorithms with 2+ occurrences`
  );
  console.log(
    `   • ${edgeAlgorithmsForFastest.length} edge algorithms total (including single occurrences)`
  );
  console.log(
    `   • ${cornerAlgorithmsForFastest.length} corner algorithms total (including single occurrences)`
  );
